...
public class AuthService {
    ...
    public UserResponse register(RegisterRequest request) {
        ...
        // Public self-registration only ever creates beneficiary accounts. Roles sent by the
        // client are never trusted (prevents privilege escalation); officer and admin accounts
        // are provisioned by seed data or by an administrator.
        if (request.getRoles() != null) {
            for (String roleStr : request.getRoles()) {
                ...
                if (!RoleType.ROLE_BENEFICIARY.name().equals(normalized)) {
                    throw new BadRequestException("Self-registration is limited to beneficiary accounts");
                }
            }
        }
